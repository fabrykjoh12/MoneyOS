...
function bfEnsure(){
  const budget=document.getElementById('budget-system');if(!budget||document.getElementById('budget-funding'))return!!budget;
  const el=document.createElement('section');el.id='budget-funding';el.className='bf-section';
  el.innerHTML=`
    ...
    <div class="bf-hero">
      <div class="bf-ring-wrap"><div id="bf-ring" class="bf-ring"><div><strong id="bf-pct">0%</strong><span>finansiert</span></div></div></div>
      <div class="bf-hero-copy"><span id="bf-month-label">NESTE MÅNED</span><h3><span id="bf-funded">—</span> av <span id="bf-full">—</span></h3><p id="bf-story">—</p><div class="bf-progress"><span id="bf-progress"></span></div></div>
    ...
    <div class="bf-quick"><div><span>HURTIGFINANSIER</span><p>MoneyOS reserverer bare det som mangler, aldri mer enn det som er ufordelt.</p></div><button data-bf-cover="minimum" type="button">Dekk minimum</button><button data-bf-cover="robust" type="button">Dekk robust</button><button data-bf-cover="full" type="button">Dekk full måned</button></div>
    <div class="bf-split">
  ...
  const month=document.getElementById('bf-month');month.value=bfTarget||bfNextMonth();month.min=bfNextMonth();
...
function bfEnsureHome(){
  const shell=document.querySelector('[data-view-panel="today"] .home-shell');if(!shell||document.getElementById('bf-home'))return!!shell;
  const anchor=document.getElementById('monthly-margin-home')||shell.querySelector('.japan-home-strip');if(!anchor)return false;
  const el=document.createElement('section');el.id='bf-home';el.className='bf-home';el.innerHTML=`<div><p class="panel-kicker">NESTE MÅNED</p><h2 id="bf-home-title">—</h2><p id="bf-home-copy">—</p></div><button type="button">Finansier →</button>`;
...
function bfRenderQuick(d){const defs={minimum:['Dekk minimum',d.remaining_to_minimum_nok],robust:['Dekk robust',d.remaining_to_robust_nok],full:['Dekk full måned',d.remaining_to_full_nok]};document.querySelectorAll('[data-bf-cover]').forEach(btn=>{const[label,need]=defs[btn.dataset.bfCover]||['Dekk',0];const usable=Math.min(Math.max(0,bfN(need)),Math.max(0,bfN(d.available_to_allocate_nok)));btn.disabled=need<=0||d.available_to_allocate_nok<=0;btn.textContent=need<=0?`${label} ✓`:usable<need?`Reserver ${bfKr(usable)}`:`${label} · ${bfKr(need)}`})}
function bfRender(){
  ...
  document.getElementById('bf-funded').textContent=bfKr(d.funded_nok);document.getElementById('bf-full').textContent=bfKr(d.full_month_nok);document.getElementById('bf-pct').textContent=`${Math.round(pct)}%`;document.getElementById('bf-progress').style.width=`${pct}%`;document.getElementById('bf-ring').style.setProperty('--bf-progress',`${pct*3.6}deg`);document.getElementById('bf-available').textContent=bfKr(d.available_to_allocate_nok);
  let story='Ingen penger er reservert til måneden ennå.';if(d.funded_nok>=d.full_month_nok&&d.full_month_nok>0)story='Hele månedsplanen er allerede finansiert med penger som finnes.';else if(d.funded_nok>=d.robust_month_nok&&d.robust_month_nok>0)story='Minimum og true expenses er finansiert. Nå gjenstår bare sparing og fri pott.';else if(d.funded_nok>=d.minimum_month_nok&&d.minimum_month_nok>0)story='Minimumsmåneden er finansiert. Neste prioritet er true expenses.';else if(d.funded_nok>0)story=`${bfKr(d.remaining_to_minimum_nok)} mangler før minimumsmåneden er dekket.`;
  ...
  const clear=document.getElementById('bf-clear');if(clear)clear.disabled=d.funded_nok<=0;bfRenderSalary(d);bfRenderQuick(d);
...
function bfRenderPreview(preview){
  const root=document.getElementById('bf-preview');if(!root)return;root.classList.remove('hidden');const used=preview.reserved_nok;root.innerHTML=`<div class="bf-preview-head"><div><span>FORDELING</span><strong>${bfKr(used)} reserveres</strong></div>${preview.left_unassigned_nok>0?`<p>${bfKr(preview.left_unassigned_nok)} blir stående ufordelt.</p>`:''}</div><div class="bf-preview-steps">${preview.steps.map((s,i)=>`<div class="bf-preview-step ${s.allocated_nok>0?'active':''}"><span>${i+1}</span><div><strong>${bfEsc(s.label)}</strong><small>${bfKr(s.needed_nok)} behov</small></div><b>${s.allocated_nok>0?`+${bfKr(s.allocated_nok)}`:'—'}</b></div>`).join('')}</div><button id="bf-apply" type="button"${used<=0?' disabled':''}>Reserver ${bfKr(used)}</button>`;document.getElementById('bf-apply')?.addEventListener('click',()=>bfApply(preview.amount_nok));
}
...
async function bfCover(level){if(!bfData)return;const need=level==='minimum'?bfData.remaining_to_minimum_nok:level==='robust'?bfData.remaining_to_robust_nok:bfData.remaining_to_full_nok;const amount=Math.min(Math.max(0,bfN(need)),Math.max(0,bfN(bfData.available_to_allocate_nok)));if(amount<=0)return;await bfApply(amount)}
async function bfApply(amount){const btn=document.getElementById('bf-apply');if(btn){btn.disabled=true;btn.textContent='Reserverer…'}try{const r=await fetch('/api/budget-funding',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'allocate_amount',month:bfTarget||bfNextMonth(),amount_nok:amount})});const body=await r.json();if(!r.ok)throw new Error(body.error||'Kunne ikke reservere');bfData=body;bfPreview=null;document.getElementById('bf-amount').value='';document.getElementById('bf-preview').classList.add('hidden');bfRender();document.dispatchEvent(new CustomEvent('moneyos:budget-updated'));document.getElementById('refresh')?.click();setTimeout(()=>bfLoad(true),650)}catch(err){if(btn){btn.disabled=false;btn.textContent=err.message}else alert(err.message)}}
async function bfAllocateSalary(id){const btn=document.getElementById('bf-use-salary');if(btn){btn.disabled=true;btn.textContent='Fordeler…'}try{const r=await fetch('/api/budget-funding',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'allocate_salary',month:bfTarget||bfNextMonth(),transaction_id:id})});const body=await r.json();if(!r.ok)throw new Error(body.error||'Kunne ikke fordele lønnen');bfData=body;bfPreview=null;bfRender();document.dispatchEvent(new CustomEvent('moneyos:budget-updated'));document.getElementById('refresh')?.click();setTimeout(()=>bfLoad(true),650)}catch(err){if(btn){btn.disabled=false;btn.textContent=err.message}}}
async function bfClear(){if(!bfData||bfData.funded_nok<=0)return;if(!confirm(`Frigi ${bfKr(bfData.funded_nok)} som er reservert til ${bfTitle(bfMonthFmt.format(bfMonthDate(bfData.month)))}?`))return;const r=await fetch('/api/budget-funding',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'clear_funding',month:bfData.month})});if(r.ok){bfData=await r.json();bfPreview=null;bfRender();document.dispatchEvent(new CustomEvent('moneyos:budget-updated'));document.getElementById('refresh')?.click();setTimeout(()=>bfLoad(true),650)}}
async function bfLoad(force=false){if(bfLoading)return;const app=document.getElementById('app');if(!app||app.classList.contains('hidden'))return;if(!bfTarget)bfTarget=bfNextMonth();if(bfTarget<bfNextMonth())bfTarget=bfNextMonth();bfLoading=true;try{for(let i=0;i<20&&!bfEnsure();i++)await new Promise(r=>setTimeout(r,80));const r=await fetch(`/api/budget-funding?month=${encodeURIComponent(bfTarget)}`,{credentials:'same-origin',cache:'no-store'});if(!r.ok)return;bfData=await r.json();bfPreview=null;bfRender()}finally{bfLoading=false}}
function bfBoot(){const app=document.getElementById('app');const run=()=>{if(app&&!app.classList.contains('hidden'))setTimeout(()=>bfLoad(),500)};run();if(app)new MutationObserver(run).observe(app,{attributes:true,attributeFilter:['class']});document.getElementById('refresh')?.addEventListener('click',()=>setTimeout(()=>bfLoad(true),900));document.addEventListener('moneyos:budget-updated',()=>setTimeout(()=>bfLoad(true),450));setTimeout(()=>bfLoad(),1600)}
bfBoot();

(()=>{
  ...
  const groups={
    today:{
      styles:['money-search.css','smart-insights.css','data-quality.css','active-month-home.css','home-runway.css'],
      scripts:['dashboard-cache.js','money-search.js','smart-insights.js','data-quality.js','active-month-home.js','home-runway.js']
    },
    money:{
  ...
  function addStyle(href){
    if(document.querySelector(`link[data-moneyos-feature="${href}"]`)||[...document.styleSheets].some(s=>String(s.href||'').endsWith('/'+href)))return;
    const link=document.createElement('link');link.rel='stylesheet';link.href=`/${href}`;link.dataset.moneyosFeature=href;document.head.appendChild(link);
  }
  function addScript(src){
    return new Promise(resolve=>{
      if(document.querySelector(`script[data-moneyos-feature="${src}"]`)||document.querySelector(`script[src="/${src}"]`)){resolve();return;}
      const s=document.createElement('script');s.src=`/${src}`;s.dataset.moneyosFeature=src;
      if(!['dashboard-cache.js','budget-lock-guard.js'].includes(src))s.type='module';
      s.onload=resolve;s.onerror=()=>{console.error(`MoneyOS feature failed: ${src}`);resolve()};document.body.appendChild(s);
    });
  }
  const pause=(ms=180)=>new Promise(r=>setTimeout(r,ms));
  async function loadGroup(name){
    if(!groups[name]||loaded.has(name))return;
    if(loading.has(name))return loading.get(name);
    const task=(async()=>{
      groups[name].styles.forEach(addStyle);
      for(const src of groups[name].scripts){await addScript(src);await pause();}
      loaded.add(name);
      document.dispatchEvent(new CustomEvent('moneyos:features-loaded',{detail:{view:name}}));
  ...
  document.addEventListener('click',event=>{
    const target=event.target?.closest?.('[data-view],[data-go]');if(!target)return;
    const view=target.dataset.view||target.dataset.go;
    if(groups[view])setTimeout(()=>loadGroup(view),250);
  });
})();